...
class MusicController {
  async CreateMusic(req, res) {
    try {
      ...
      return res
        .status(400)
        .json({ success: false, Message: "Chưa có dữ liệu file" });
    }
  }
  ...
  async Update_Music_ForUser(req, res) { // Tăng số lượt view của bản nhạc
    const { id } = req.body;
    try {
      {
  ...
  async Get_All_Music(req, res) {
    try {
      const getALLProduct = await Music.find()
        .populate("userID")
        .populate("ArrayLike._id", "name account_image") // Điền ArrayLike với dữ liệu người dùng
        .populate("ArrayComment._id", "name account_image") // Điền ArrayComment với dữ liệu người dùng
